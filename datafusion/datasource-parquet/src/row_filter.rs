...
pub(crate) struct FilterCandidate {
    ...
    /// Estimated fraction of rows retained by this predicate. This is only
    /// populated for dynamic integer `IN` filters, whose runtime values make
    /// their selectivity cheap to estimate.
...
fn integer_scalar_value(value: &ScalarValue) -> Option<i128> {
    match value {
        ...
        ScalarValue::Int32(Some(value)) => Some((*value).into()),
...
pub fn build_row_filter(
    ...
    if reorder_predicates {
        candidates.sort_unstable_by(|a, b| {
            predicate_ordering_cost(a).total_cmp(&predicate_ordering_cost(b))
        });
    }
...
/// For serial predicates the expected work is minimized by ordering on
/// `evaluation cost / rejection probability`. Compressed bytes approximate
/// evaluation cost; dynamic integer `IN` density approximates selectivity.
fn predicate_ordering_cost(candidate: &FilterCandidate) -> f64 {
    let rejection = 1.0 - candidate.estimated_selectivity.unwrap_or(0.0);
    candidate.required_bytes as f64 / rejection.max(f64::EPSILON)
}
...
mod test {
    ...
    use datafusion_physical_expr::expressions::{
        Column as PhysicalColumn, DynamicFilterPhysicalExpr, in_list, lit,
    };

    #[test]
    fn estimates_dynamic_integer_in_list_selectivity() {
        let schema = Schema::new(vec![Field::new("a", DataType::Int32, true)]);
        let column = Arc::new(PhysicalColumn::new("a", 0)) as Arc<dyn PhysicalExpr>;
        let list = vec![lit(10_i32), lit(11_i32), lit(19_i32)];
        let inner = in_list(Arc::clone(&column), list, &false, &schema).unwrap();
